...
One square bracket `[]` will retrieve the box, while double square brackets would upack the box
...
There are two flow structues for loops, i.e., `for` loops and `while` loops
...
One can also iterate through a vector using its indecies
...
Repeats executing the code enclosed while a condition is satisified
...
1- Find the if there is a significant difference between TNBC and nonTNBC samples based on these genes [ESR1, PGR, ERBB2] and show that in a plot.  
2- Compute the pairwise correlation between all the samples and show that in a plot. 

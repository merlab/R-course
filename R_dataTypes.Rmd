---
title: "R for beginners Notebook"
output:
  ioslides_presentation: default
  slidy_presentation: default
  beamer_presentation: default
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


# R data-types and structures

## Variable

A named container or storage that is used to reference a value in the code.

- This value can be of many types:
    + numeric: ```1, 2, 3```
    + character: ```"a", "b", "c"```
    + logical: ```TRUE, FALSE``` or simply ```T, F```
    + Integers, complex, and factors
    
## Variable

* Constraints on names:
    + must starts with a letter or a dot not followed by a number.
    + can only contain letters, numbers, underscore and the dot character "."
    
    ``` var1, .var1, var.name, v1 ```
    ``` 1v, .1v, var-1, var#1 ``` 

## Variable

```<-``` and ```=``` are used for assignment of values in R 

```{r}
  a <- 2
  b <- 3
  a+b
```

* Note that R variables are dynamically typed
```{r}
  a <- 2
  a <- "hello"
```


## Vector
A `vector` is the simplest data structure in R. it represents a container for a set of elements of the same data type. 
-  Examples:
```{r}
  c(1,3,5)
  length(c("test"))
  class(c(TRUE,TRUE,FALSE))
```

## Vector
`c` function is used to concatenate vectors.
```{r}
  a <- c(1,3,5)
  b <- c(2,4,6)
  c <- c(a,b)
  c
```

Remember: vectors have a single data type
```{r}
  a <- c(1,3,5)
  b <- c("a","b")
  c(a,b)
```

## Vector
Arithmetics are performed on vectors member-by-member
```{r}
  a <- c(1:3) # R way to create a sequence of numbers
  a
  a + 1
```

## Vector
and this extends to operations between vectors
```{r}
  a <- c(1,3,5)
  b <- c(2,4,6)
  a+b
```

## Vector
What will happen if the two vectors are of different lengths?
```{r}
  a <- c(1,3,5)
  b <- c(2,4,6,8,10)
  c <- a+b
```

## Vector
Exercise: Create a vector of Logical values, assign it to a variable. Create a vector of Integers and assign to variables. 

Try subtracting the two vectors. What happened?


## Vector
Retrieving elements from a vector is done by `indexing`

-  There are many ways to use indexing and the simplest is `numeric indexing`, i.e. a number that represents the position of an element in a vector.

-  `[]` is used for indexing. In R, indexing starts from `1`

```{r}
  a <- c(1,3,5,7,9)
  a[2]
  a[c(3,5)]
```

## Vector - Indexing
`indexing` can be used also for changing values in a vector

```{r}
  a <- c(1,3,5,7,9)
  a[2] <- 4 
  a
  a[c(3,5)] <- c(6,10)
  a
  a[-1] # negative indexing is used to remove elements
```

## Vector - Indexing
Logical values can be used for `indexing` too.

```{r}
  a <- c(1,3,5,7,9)
  a[c(T,F,T,F,T)]
  a[c(F,F,T,F,T)] <- c(6,10)
  a
  b <- a[c(T,F)] # what will happen??
```

## Named Vectors 

R allows us to give names to each element in a vector, and use them to index:
```{r}
a <- c(2,4,6,8,10)
names(a) <- c("A", "B", "C", "D", "E")
a[c("A", "D")]
```

## Matrix
A `matrix` is a collection of data elements arranged in a two-dimensional rectangular layout
```{r}
a = matrix( 
   c(2, 4, 3, 1, 5, 7), # the data elements 
   nrow=2,              # number of rows 
   ncol=3)             # number of columns 
a
a[2,3] # note the ',' to indiacte row and column indexing
```

## Matrix - Names

Matrix can have row and/or column names:

```{r}
a <- matrix(data=1:10, nrow=2, ncol=5)
rownames(a) <- c("A", "B")
a
a["B", c(2,5)]
a["A",]
```


## List
A `list` is a generic vector that can contain multiple data types. Think of it as a vector of boxes, you can put anything in the box.
```{r}
b <- list(1, "a", c(TRUE, FALSE))
b
```


## List - indexing

One square bracket `[]` will retrieve the box, while double square brackets would upack the box

```{r}
n = c(2, 3, 5) 
s = c("aa", "bb", "cc", "dd", "ee") 
b = c(TRUE, FALSE, TRUE, FALSE, FALSE) 
a <- list(n,s,b)
a[2]
a[[2]]
```


## List - indexing

```{r}
n = c(2, 3, 5) 
s = c("aa", "bb", "cc", "dd", "ee") 
a <- list(n,s)
a[[2]][3]
```

## List - indexing

Named lists provide another way to index them using the operator `$`

```{r}
n = c(2, 3, 5) 
s = c("aa", "bb", "cc", "dd", "ee") 
a <- list("numbers"=n,"strings"=s)
a$numbers
a[["strings"]]
```

## Dataframe
A `dataframe` is used for storing data tables. It represents a list of vectors of equal length.

```{r}
n = c(2, 3, 5) 
s = c("aa", "bb", "cc") 
b = c(TRUE, FALSE, TRUE) 
df = data.frame(n, s, b)       # df is a data frame
df
df$s
df[["b"]]
```

## Dataframe

```{r}
n = c(2, 3, 5) 
s = c("aa", "bb", "cc") 
b = c(TRUE, FALSE, TRUE) 
df = data.frame(n, s, b)       # df is a data frame
df$b
df[["s"]]
df = data.frame(n, s, b,stringsAsFactors = F) 
df$s
```

## R data-types and structures

These are the main data types and structures in R.

We still need to talk about factors!

# R operations

## R Arithmetic operators

```{r, echo=TRUE, results='hide'}
x <- 3
y <- 5
+ x       # 3
- x       # -3
x + y     # 8
x - y     # -2
x * y     # 15
x / y     # 0.6
x ^ y     # 243
x %% y    # 3
x %/% y   # 0
```

## R Relational operators
```{r, echo=TRUE, results='hide'}
x <- 3
y <- 5
x < y    # TRUE
x > y    # FALSE
x <= y   # TRUE
x >= y   # FALSE
x == y   # FALSE
x != y   # TRUE

```


## R Logical operators

```{r, echo=TRUE, results='hide'}
x <- TRUE
y <- FALSE
!x       # FALSE  -- NOT
x && y   # FALSE  -- Logical AND
x || y   # TRUE   -- Logical OR
```


```{r}
x <- c(TRUE,TRUE,FALSE)
y <- c(TRUE,FALSE,FALSE)
x & y   # element-wise AND
x | y   # element-wise OR
```

# R control flow structures

## if-else 

This is a conditional flow by which the following steps of the program are dependent on the outcome of the condition.

```{r}
if(3>2){
  print("Yes") 
}
```

```{r}
if(3<2){
  print("Yes") 
}else{
  print("No")
}
```

## if-else 

```{r}
if(3<2){
  print("No") 
}else if(3>2){
  print("Yes")
}else{
  print("Maybe")
}

ifelse(3>2,"yes","No")
```

## Loops

Loops are used to repeat a block of code until a condition is satisfied 

There are two flow structues for loops, i.e., `for` loops and `while` loops


## for loops

The most common use is to iterate through the elements of a vector

```{r}
myVec <- c("A", "B", "C")
for(letter in myVec){
  print(letter)
}
```

## for loops

One can also iterate through a vector using its indecies

```{r}
myVec <- c(2, 5, 21)
for(i in seq_along(myVec)){
  print(myVec[i] + 3)
}
```
Note: seq_along is a built-in function. You might be tempted to do `1:length(myVec)`. This breaks when `length(myVec)` is accidentally 0.

## while loops

Repeats executing the code enclosed while a condition is satisified

```{r}
myVec <- c("a","b","c")
i <- 1
while(i <= length(myVec)){
  print(myVec[i])
  i <- i + 1
}
```

# functions

## functions

R does everything by copying. What do we mean by that?

```{r, results="hide"}
a <- 3
b <- a
a <- 2
a
b
```


## functions

R does everything by copying. What do we mean by that?

```{r}
a <- 3
b <- a
a <- 2
a
b
```


## functions

-  A `function` is a block of code that can be used multiple times. It is a way to structure the code.

-  R provides many built-in functions that ease writing programs and executing codes.

-  Examples: `print`, `:`, `c`, `in`

## functions

R also provides users with the ability to write their own custom functions

```{r}
printThanks <- function(){
  return("Thank you very much for running this code")
}

printThanks
```

## functions

You could add also parameters to your function
```{r}
isEven <- function(x){
  if(x%%2==0){
    return(TRUE)
  }
  return(FALSE)
}

isEven(4)
isEven(3)
```

## functions

Variables defined inside a function are local to that function.

```{r, results="hide"}
x <- 5
testScope <- function(x){
  return(x+1)
}
testScope(4)
testScope(x)
x
```

## functions

Variables defined inside a function are local to that function.

```{r}
x <- 5
testScope <- function(x){
  return(x+1)
}
testScope(4)
testScope(x)
x
```



## *apply

Functions in R become super useful together with the built-in apply functions. *apply helps you apply a function to each element of a vector or row/column of a matrix. 


There are 3 main apply functions: `lapply`, `sapply` and `apply`. `lapply` - l stands for list, will return a list. `sapply` s stands for simple, will try to simplify the list. `apply` is `sapply` for matrices and data.frames. 

## sapply

Here is an sapply example:
```{r}
isEven <- function(x){
  if(x%%2==0){
    return(TRUE)
  }
  return(FALSE)
}

myVec <- c(4,3,2,1)
sapply(myVec, isEven)
```

## apply

`apply`, unlike sapply and lapply takes 3 arguments: the data to apply over, which dimensions to apply over, and the function to apply. 

Heres a silly example, using the built-in `sum` function, which returns the sum of a vector:
```{r, results="hold"}
my.mat <- matrix(1:4, nrow=2, ncol=2)
my.mat
apply(my.mat, 1, sum)
apply(my.mat, 2, sum)
```

## functions
R has many built-in functions that can ease writing programs and you can find their syntax by using `?` operator

Examples:
```{r}
?mean
?rownames
?read.csv
```











